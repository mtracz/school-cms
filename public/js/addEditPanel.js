...
$("#preview_button").on("click", function () {
	...
	} else if (!validateTitle() || !validateContent()) return false;
...
$("#reedit_button").on("click", function () {
	if (customPanel.length > 0) hideContentToolsIcon();
});
...
function sendPageData() {
	...
	if (customPanel.length > 0) payload_form.append("content", custom_content);else payload_form.append("content", form_content);
	...
	if ($(".ui.segment.preview").find(".banner.panel").length > 0) payload_form.append("header", "");else payload_form.append("header", form.header.value);
